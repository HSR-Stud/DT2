...
\subsection{Eigenschaften Sequenzieller systeme}
	\begin{multicols}{2}
		\begin{itemize}
		  ...
		  \item Grundelemnte für Ged"achtnis: FlipFlop
		  \item Seq. Systeme können mit Zustandsdiagrammen dargestellt werden
...
\subsection{Zustandcodes}
	...
	\begin{itemize}
		...
		\item Zustände werden der Reihe nach binär durch nummeriert.
		\item Benötigte Anzahl Speicherstellen $k$ für eine Anzahl Zuständer $p$:\\
		$p\leq 2^k$ | $k=\log_2(p)=\frac{\log_{10}(p)}{\log_{10}(2)}$
		\item Anzahl Möglicher Speicherstellen:\\
		$q=\frac{(2^k)!}{(2^k-p)!}$
