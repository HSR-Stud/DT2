...
\begin{tabular}{|p{6cm}|p{6cm}|p{6cm}|}
  ...
  s[i+1]:=G(x[i],s[i])$ \\
  \hline
  Die Ausgabe ändert unabhängig vom Clock. Sequentielle Komposition erzeutgt
...
\subsection{Taktfrequenz}
  ...
  längsten Pfad, dadurch wird $f_{clkMax} $ grösser, gleichzeitig aber $T_{tot} $ (Latency) grösser.
  \subsubsection{Bestimmung des kritischen Pfades}
    \begin{enumerate}
      ...
      Gatters mit $ max\{Eingänge\}+t_p $ beschriften.
      \item Sind noch unbeschriftete Ausgangsleitungen vorhanden, dann nochmals
      2 Durchführen.
      \item Die Länge des längsten Pfades ist $max\{Ausgänge\}$
    \end{enumerate}
